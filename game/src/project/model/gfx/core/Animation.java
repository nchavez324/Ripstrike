...
public class Animation {
    ...
    public synchronized void reverseStart(){
    	 movieTime = totalTime;
    ...
    public synchronized void update(long timePassed) {
        ...
        if (numFrames > 1 && !stopped) {
        	...
        	if (numFrames > 1 && !stopped) {
                movieTime -= timePassed; // Updates movieTime.
                if ((movieTime <= 0) && !once) {// Checks to see if the sequence should                         // be restarted.
                    reverseStart();
                }else if((movieTime <= 0) && once){
                	stopped = true;
                }
                while (movieTime <= sceneIndex * frameTime) { // Checks to move                                              // next frame.
                    sceneIndex--;
                }
            }
        }
