...
public class CollisionTile implements java.io.Serializable{
	...
	public void draw(Graphics2D g, Vector2 mapPosition, int x, int y) {
		...
		if (type == Type.SLOPE) {
			Vector2 p1 = new Vector2(mapPosition.X + CollisionMap.SIZE * x - Global.camera.x,
					mapPosition.Y + CollisionMap.SIZE * y - Global.camera.y);
			Vector2 p2 = new Vector2(mapPosition.X + CollisionMap.SIZE * x - Global.camera.x,
					mapPosition.Y + CollisionMap.SIZE * y - Global.camera.y);
			calcPts(p1, p2);
			g.drawLine((int) (p1.X), (int) (p1.Y), (int) (p2.X), (int) (p2.Y));
		} else if (type == Type.BLOCK) {
			g.drawRect(
					(int) (mapPosition.X + CollisionMap.SIZE * x - Global.camera.x),
					(int) (mapPosition.Y + CollisionMap.SIZE * y - Global.camera.y),
